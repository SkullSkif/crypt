...
//             s[i] = ((k*(static_cast<int>(hash_bytes[i])))+(x*r))%q;
//         }      
...
//         std::vector<int> s;
//         int s_element;
//         while (sign >> s_element) {
//             s.push_back(s_element);
//         }
//         sign.close();    
        
//         auto invalid_element = std::find_if(s.begin(), s.end(), [q](int element) { return element <= 0 || element >= q;} );

//         if (invalid_element == s.end()) {
//             std::cout << "OK!\n";
//         } else {
//             std::cout << "ERROR!\n";
//         }
...
//             // h_inv = invert_c(static_cast<int>(hash_bytes[i]), q);
//             // u1 = (s[i] * h_inv) % q;
//             // u2 = (-r * h_inv) % q;
...
//         std::random_device rd;
//         std::mt19937 gen(rd());
//         std::uniform_int_distribution<> dist(700, 10000);   
//         q = dist(gen);
//         while (ferm(q) <= 0)
//             q = dist(gen);    
//         int b = 17;
...
//         int k;
//         int r = 0;
//         std::vector<int> s(hash_bytes.size());
...
//                 s[i] = (x * r + k * static_cast<int>(hash_bytes[i])) % q;
//                 if (s[i] == 0) break; 
//             }      
//             it = std::find(s.begin(), s.end(), 0);
//         }
...
//         // Проверка диапазонов
//         auto invalid_element = std::find_if(s.begin(), s.end(), 
//             [q](int element) { return element <= 0 || element >= q; });
...
//         std::cout << "Testing invert_c:\n";
//         for (int i = 1; i < 10; i++) {
//             int inv = invert_c(i, q);
//             if ((i * inv) % q != 1) {
//                 std::cout << "ERROR: invert_c not working for " << i << "\n";
//             }
//         }
        
//         // Проверка работы fast_power
//         std::cout << "Testing fast_power:\n";
//         int test = fast_power(2, 3, 100);
//         if (test != 8) {
//             std::cout << "ERROR: fast_power not working\n";
//         }
...
//             w = invert_c(s[i], q);
//             if (w == 0) {
//                 std::cout << "INCORRECT_SIGN! (cannot invert s)\n";
//                 all_valid = false;
//                 continue;
//             }
            
//             // Вычисление u1 и u2
//             u1 = (static_cast<int>(hash_bytes[i]) * w) % q;
//             // u2 = (r * w) % q;
//             // u2 = (q - (r * w) % q) % q;
//             u2 = (-r * w) % q;
//             if (u2 < 0) u2 += q;
...
//             if (v != r) {
//                 std::cout << "INCORRECT_SIGN for byte " << i << "!\n";
//                 std::cout << "  Byte value: " << static_cast<int>(hash_bytes[i]) << "\n";
//                 std::cout << "  w: " << w << ", u1: " << u1 << ", u2: " << u2 << "\n";
//                 std::cout << "  v: " << v << ", r: " << r << "\n";
//                 all_valid = false;
//             } else {
//                 std::cout << "Byte " << i << " signature VALID\n";
//             }
//         }
...
//         std::random_device rd;
//         std::mt19937 gen(rd());
//         std::uniform_int_distribution<> dist(10000, UINT16_MAX);   
//         q = dist(gen);
//         while (ferm(q) <= 0)
//             q = dist(gen);    
//         int b = 17;
...
//             for (size_t i = 0; i < hash_bytes.size(); ++i) {
//                 // ПРИВЕДЕНИЕ ХЕША К ДИАПАЗОНУ [0, q-1]
//                 int h_i = static_cast<int>(hash_bytes[i]) % q;
//                 s[i] = (x * r + k * h_i) % q;
//                 if (s[i] == 0) break; 
//             }      
//             it = std::find(s.begin(), s.end(), 0);
...
//             // Вычисление u1 и u2
//             u1 = (h_i * w) % q;
//             u2 = (q - (r * w) % q) % q;  // Отрицательное по модулю
...
//             if (v != r) {
//                 std::cout << "INCORRECT_SIGN for byte " << i << "!\n";
//                 std::cout << "  Byte value: " << static_cast<int>(hash_bytes[i]) << " -> h_i: " << h_i << "\n";
//                 std::cout << "  w: " << w << ", u1: " << u1 << ", u2: " << u2 << "\n";
//                 std::cout << "  a_u1: " << a_u1 << ", y_u2: " << y_u2 << "\n";
//                 std::cout << "  temp: " << temp << ", v: " << v << ", r: " << r << "\n";
//                 all_valid = false;
//             } else {
//                 std::cout << "Byte " << i << " signature VALID\n";
//             }
...
// uint64_t hash_to_int(const std::string& hash, uint32_t q) {
    
//     uint64_t result = 0;
    
//     for (size_t i = 0; i < hash.size(); ++i) {
//         result = (result * 256 + static_cast<unsigned char>(hash[i])) % q;
//     }
