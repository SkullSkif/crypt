...
{
    if (p <= 1) {
        throw std::invalid_argument("Modulus p must be greater than 1");
    }
    if (a < 0) a = a % p + p;
    if (x < 0) throw std::invalid_argument("Exponent x must be non-negative");
    ...
    while (x > 0) {
        if (x % 2 == 1) {
            ...
            if (y > LLONG_MAX / current) {
                throw std::overflow_error("Integer overflow in multiplication");
            }
            y = (y * current) % p;
        }
        
        // Проверка переполнения при возведении в квадрат
        if (current > LLONG_MAX / current) {
            throw std::overflow_error("Integer overflow in squaring");
        }
...
// uint64_t gen_euclid_v(uint64_t* a, uint64_t* b)
// {
//     // if(*a < *b)
//     //     return -1;
//     if (*a < (uint64_t)-1 || *b < (uint64_t)-1)
//         return -1;
//     if (*a == (uint64_t)0 || *b == (uint64_t)0) {
//         *a = rand() + 1;
//         *b = rand() + 1;
//     }
//     std::random_device rd;
...
//     std::uniform_int_distribution<> dist(1, 1000);
//     if (*a == (uint64_t)-1 || *b == (uint64_t)-1) {
//         uint64_t flag = 1, flag_a = 1, flag_b = 1;
//         while (flag) {
//             if (flag_a)
//                 *a = dist(gen);
//             if (flag_b)
...
void FIPS(int type){
    ...
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dist(1000, 8000);   
    q = dist(gen);
    while (ferm(q) <= 0)
        q = dist(gen);    
    uint32_t max_b = 0xFFFF / q;
   ...
   while((r==0) || (s==0)){
    ...
    uint32_t k_inv = invert_c(k, q);
    uint64_t s_temp = (static_cast<uint64_t>(k_inv) * (h + static_cast<uint64_t>(x) * r)) % q;
    s = static_cast<uint32_t>(s_temp);    
    ...
    open.close();
        ...
        uint32_t a_u1, y_u2, temp;
        uint32_t s_inv;
        s_inv = invert_c(static_cast<uint32_t>(s), q);
        u1 = mul(h,s_inv,q);
        u2 = mul(r, s_inv, q);
        // h_inv = invert_c(h,q);
        // u1 = mul(s, h_inv, q);
...
// uint64_t invert_v(uint64_t d, uint64_t fi)
// {
//     uint64_t x, y;
//     x = d; y = fi;
//     uint64_t g = gen_euclid_v(&x, &y);
//     if (g != 1) {
//         return -1;
//     }
//     return (x % fi + fi) % fi;
...
//         std::ofstream server_key_write("Server");
//         server_key_write << c;
//         server_key_write.close();
// }

// void alice_set_Blind_vote(char voice){
//     uint32_t rnd = 0;
//     std::random_device rd;
//     std::mt19937 gen(rd());
...
long long fast_power_v(long long a, long long x, long long p) {
    if (p <= 1) {
        throw std::invalid_argument("Modulus p must be greater than 1");
    }
    if (a < 0) a = a % p + p;
...
uint64_t gen_euclid_vv(uint64_t *a, uint64_t* b) {
    if (*a == 0 || *b == 0) {
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dist(1, 1000);
...
// uint64_t md5_hash_to_int(uint64_t n) {
//     // Создаем MD5 хэш от числа n
//     unsigned char digest[MD5_DIGEST_LENGTH];
//     std::string input = std::to_string(n);
//     MD5((unsigned char*)input.c_str(), input.length(), digest);
    
//     // Преобразуем первые 8 байт в 64-битное число
//     uint64_t result = 0;
...
uint64_t fast_power_v(uint64_t a, uint64_t x, uint64_t p) {
    if (p <= 1) {
        throw std::invalid_argument("Modulus p must be greater than 1");
    }
    if (a < 0) a = a % p + p;
...
void server_set_Blind_vote() {
    ...
    // // Выбираем открытую экспоненту d (обычно 65537)
    // d = 65537;

        std::uniform_int_distribution<> distD(1, 10000);
        d = distD(gen);
        uint64_t c1 = d;
        uint64_t c2 = fi;
        while (c1 < fi) {
            if (gen_euclid_vv(&c1, &c2) == 1) {
                d = c1;
                break;
            }
            c1 = distD(gen);
...
std::pair<uint64_t, uint64_t> alice_set_Blind_vote(char voice) {
    ...
    uint64_t n = (static_cast<uint64_t>(rnd) << 32) | v;
    std::cout << "Alice: Generated n = " << n << std::endl;
    ...
    auto hash_string = md5(std::to_string(n));
    uint64_t h = hash_to_int(hash_string, N);
    std::cout << "Alice: Hash h = " << h << std::endl;
    

    
    std::cout << "Alice: Loaded public key: N = " << N << ", d = " << d << std::endl;
    
    uint64_t r;
    std::uniform_int_distribution<uint64_t> dist_r(2, 10000);
    do {
        r = dist_r(gen);
    } while (gen_euclid_v(r, N) != 1);
    
    std::cout << "Alice: Generated blind factor r = " << r << std::endl;
    
    uint64_t r_pow_d = fast_power_v(r, d, N);
    uint64_t h_bar = (h % N) * (r_pow_d % N) % N;
    
    std::cout << "Alice: Sending blinded h_bar = " << h_bar << " to server" << std::endl;
    
    // В реальной системе здесь была бы отправка h_bar серверу
    // будем эмулировать работу сервера
    ...
    uint64_t s_bar = fast_power_v(h_bar, c, N);
    std::cout << "Server: Signed blinded message s_bar = " << s_bar << std::endl;
    
    // Алиса снимает кодирование
    uint64_t r_inv = invert_v(r, N);
    uint64_t s = (s_bar * r_inv) % N;
    
    std::cout << "Alice: Removed blinding, signature s = " << s << std::endl;
...
bool server_verify_vote(uint64_t n, uint64_t s) {
    ...
    // Проверка
    uint64_t s_pow_d = fast_power_v(s, d, N);
    
    bool valid = (h % N) == (s_pow_d % N);
    
    std::cout << "\nServer verification:" << std::endl;
    std::cout << "  n = " << n << std::endl;
    std::cout << "  s = " << s << std::endl;
    std::cout << "  h = " << h << std::endl;
    std::cout << "  s^d mod N = " << s_pow_d % N << std::endl;
